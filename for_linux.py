...
def any_msg(message):
	...
	if message.text == 'Привет':
		...
		f = open("/bot_example/Fails/" + idname, "rb")
		bot.send_document(message.chat.id, f)
		schedule(idname, idname_photo)
		bot.send_message(message.from_user.id,'Зависимость самочувствия от температуры:')
		bot.send_photo(message.from_user.id, photo=open('/bot_example/Pictures_temp/' + idname_photo, 'rb'))
		bot.send_message(message.from_user.id,'Зависимость самочувствия от атмосферного давления:')
		bot.send_photo(message.from_user.id, photo=open('/bot_example/Pictures_Atm.davl/' + idname_photo, 'rb'))
	else:
...
def resh(a, b, idname):
	global df
	df = pd.read_csv('/bot_example/Fails/' + idname)
	if len(df) == 0:
		i = 0
	...
	df.to_csv('/bot_example/Fails/' + idname, index=False)
def weather():
...
def newtable(idname):
	with open("/bot_example/Fails/" + idname,"w",newline="") as file_writer:
...
def schedule(idname, idname_photo):
	data = pd.read_csv('/bot_example/Fails/' + idname,sep=r'\s*,\s*', header=0, encoding='utf8', engine='python')
	...
	plt.savefig('/bot_example/Pictures_temp/' + idname_photo)
	...
	plt.savefig('/bot_example/Pictures_Atm.davl/' + idname_photo)
